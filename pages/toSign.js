...
class ToSign extends Component {
  ...
  renderDeanItems() {
    ...
    const items = this.props.certificateItems1.map((certificate, index) => {
      ...
      if(this.props.certificateItems2[index][0]==false){
      return {
        ...
        description: (
          ...
          <Link route={`/sign/${this.props.allCertificates[index]}`}>
          <a className='vows-link'>{ `Signing link`}</a>
        </Link>
        </div>
          ),
        fluid: true
  ...
  renderViceItems() {
    ...
    const items = this.props.certificateItems1.map((certificate, index) => {
      ...
      if(this.props.certificateItems2[index][1]==false){
      return {
        key: this.props.allCertificates[index],
        color: colors[index % colors.length],
        header: `${certificate[3]} Certificate issued to ${certificate[0]}`,
  ...
  renderAcademicItems() {
    ...
    const items = this.props.certificateItems1.map((certificate, index) => {
      ...
      if(this.props.certificateItems2[index][2]==false){
      return {
        key: this.props.allCertificates[index],
        color: colors[index % colors.length],
        header: `${certificate[3]} Certificate issued to ${certificate[0]}`,
                              ...
                              if (isSignedIn === true) {
                                if(firebase.auth().currentUser.email=='dean@certdapp.com'){
                                  return (
                                    ...
                                    <div>
                                    <Container>
                                        <a name="certificates"><h2 className='Certificates-Title'>Certificates to Sign</h2></a>
